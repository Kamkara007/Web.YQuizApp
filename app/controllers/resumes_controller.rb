class ResumesController < ApplicationController
  ...
  def create
    @resume = @course.resumes.build(resume_params)
    @resume.user = current_user
    respond_to do |format|
      if @resume.save
        format.html { redirect_to course_path(@course), notice: "Resume was successfully created." }
  ...
  private
    ...
    def set_resume
      @resume = @course.resumes.friendly.find(params[:id])
    end
    ...
    def resume_params
      params.require(:resume).permit(:title, :slug, :user_id, :course_id)
    end
end
